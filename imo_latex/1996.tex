...
\begin{enumerate}
\item[(a)] Show that the task cannot be done if $r$ is divisible by 2 or 3.
\item[(b)] Prove that the task is possible when $r = 73$.
\item[(c)] Can the task be done when $r = 97$?
\end{enumerate}
...
\begin{enumerate}
\item[(a)] $x_0 = x_n = 0$.
\item[(b)] For each $i$ with $1 \leq i \leq n$, either $x_i - x_{i-1} = p$ or $x_i - x_{i-1} = -q$.
\end{enumerate}
